...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    if (cmd.length()>0) {
      ...
      */
      ...
      else if (cmd=="serial") {  //以Uart傳送指令到Arduino Uno
        Serial.print(Command.substring(8)); 
      }      
      else {
...
static esp_err_t status_handler(httpd_req_t *req){
    ...
    *p++ = '{';
    p+=sprintf(p, "\"flash\":0,");  
    p+=sprintf(p, "\"speedL\":255,"); 
    p+=sprintf(p, "\"speedR\":255,"); 
    p+=sprintf(p, "\"turnRatio\":0.3,");           
    ...
    <body>
        <section class="main">
            ...
            <section id="buttons">
                <table>
                  <tr>
                    <td align="center"><button id="restart" onclick="try{fetch(document.location.origin+'/control?restart');}catch(e){}">Restart</button></td>
                    <td align="center"><button id="get-still">Get Still</button></td>
                    <td align="center"><button id="toggle-stream">Start Stream</button></td>
                  </tr>                
                  <tr>
                    <td><input type="checkbox" id="nostop" onclick="noStopControl();">No Stop</td>
                    <td></td>
                    <td></td>
                  </tr>
                  <tr>
            ...
            <div id="content">
                <div id="sidebar">
                    ...
                    <nav id="menu"> 
                        ...
                        <div class="input-group" id="turnRatio-group">
                            <label for="turnRatio">Turn Ratio</label>
                            <div class="range-min">0</div>
                            <input type="range" id="turnRatio" min="0" max="1" step="0.1" value="0.3" class="default-action">
                        ...
                        <div class="input-group" id="vflip-group">
                            <label for="vflip">H-Mirror</label>
                            <div class="switch">
                        ...
                        <div class="input-group" id="hmirror-group">
                            <label for="hmirror">V-Flip</label>
                            <div class="switch">
        ...
        <script>
          document.addEventListener('DOMContentLoaded', function (event) {
            ...
            function updateConfig (el) {
              ...
              if (el.id=="speedL") {
                uartCommand("?speedL="+el.value);
                return;
              } else if (el.id=="speedR") {
                uartCommand("?speedR="+el.value);
                return;
              } else if (el.id=="turnRatio") {
                uartCommand("?turnRatio="+el.value);
                return;
              }
          ...
          function uartCommand(command) {
            try{
              var url = location.origin+'/control?serial='+command;
              console.log(url);
              fetch(url);
            }
            catch(e){}
          ...
          stream.onload = function (event) {
            ...
            Timer = setInterval(function(){
              canvas.setAttribute("width", stream.height);
              canvas.setAttribute("height", stream.width);
              context.translate(canvas.width/2,canvas.height/2); 
              context.rotate(-Math.PI/2); 
              context.drawImage(stream,-stream.width/2,-stream.height/2); 
              context.rotate(Math.PI/2); 
              context.translate(-canvas.width/2,-canvas.height/2); 
            },200);
          }
