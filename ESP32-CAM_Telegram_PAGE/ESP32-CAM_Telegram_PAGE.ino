...
<html>
    ...
    <script>
      ...
      sendimage.onclick = function (event) {
        ...
        getFeedback(location.origin+'/?SendCapturedImage='+token.value+';'+chat_id.value+';'+(new Date().getFullYear()*10000000000+(new Date().getMonth()+1)*100000000+new Date().getDate()*1000000+new Date().getHours()*10000+new Date().getMinutes()*100+new Date().getSeconds()+new Date().getSeconds()*0.001).toString());
      }      
...
void ExecuteCommand()
{
  ...
  if (cmd=="your cmd") {
    ...
    Feedback=sendCapturedImage2Telegram(P1,P2);
