...
<html>
    <head>
        ...
        <title>Face Recognition (face-api.js)</title>
        <style>
    ...
    <body>
        <section class="main">
            <section id="buttons">
                <table>
                <tr><td colspan="3"><canvas id="canvas" width="0" height="0"></canvas><canvas id="canvas_recognize" style="display:none"></canvas></td></tr>
                <tr><td><button id="restart" onclick="try{fetch(document.location.origin+'/control?restart');}catch(e){}">Restart</button></td><td><button id="get-still" style="display:none">Get Still</button><button id="getStill">Get Still</button></td><td><button id="toggle-stream" style="display:none"></button><button id="recognize">Recognize</button></td></tr>
                </table>
            </section>
            <figure>
  ...
  <script>
    ...
    getStill.onclick = function (event) {
      ...
      ShowImage.src=location.origin+'/capture?_cb='+Math.random();
    }
    ...
    function error_handle() {
      ...
      clearInterval(myTimer);
      if (restartCount<=2) {
        message.innerHTML = "Get still error. Restart ESP32-CAM "+restartCount+" times.";
        myTimer = setInterval(function(){getStill.click();},6000);
      }
      else
        message.innerHTML = "Get still error. Please reload the page and check ESP32-CAM.";
    }    
