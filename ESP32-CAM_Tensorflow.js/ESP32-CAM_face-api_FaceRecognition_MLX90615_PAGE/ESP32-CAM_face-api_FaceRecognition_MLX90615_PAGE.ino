/*
ESP32-CAM Face Recognition, MLX90615 (face-api.js)
Author : ChungYi Fu (Kaohsiung, Taiwan)  2020-4-8 23:30
...
http://192.168.xxx.xxx?temperature
...
String myLineNotifyToken = "xxxxx";
...
SlowSoftI2CMaster si = SlowSoftI2CMaster(2, 13, true);  //ESP32  SDA:P32, SCL:P33
...
void ExecuteCommand()
{
  ...
  if (cmd=="your cmd") {
    ...
    Feedback = getMLX9061X(0, 0.00, 0x5B ,0x27);
  }
  else {
...
static const char PROGMEM INDEX_HTML[] = R"rawliteral(
  ...
  <tr>
    ...
    <td><input type="button" id="detect" value="Detect" style="display:none"></td>  
  </tr>
  <tr>
    <td>MirrorImage</td> 
    <td colspan="2">  
  ...
  <script>
    ...
    async function DetectImage() {
      ...
      $.ajax({url: document.location.origin+'/?temperature', success:function(res)
      {
        message.innerHTML = res+" °C";
      }, async: false});      
      ...
      results.forEach((result, i) => {
        ...
        if (result.distance<=distanceLimit)
          drawBox = new faceapi.draw.DrawBox(box, { label: result.toString()+", "+message.innerHTML});
        else
          drawBox = new faceapi.draw.DrawBox(box, { label: "unknow("+(Math.round(result.distance*100)/100).toString()+"), "+message.innerHTML});
