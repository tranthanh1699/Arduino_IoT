...
static const char PROGMEM INDEX_HTML[] = R"rawliteral(
  ...
  <script>
    ...
    function DetectImage() {
      Model.classify(ShowImage).then(Predictions => {
        var objectCount=0;
        ...
        if (Predictions.length>0) {
          ...
          for (var i=0;i<Predictions.length;i++) {
            ...
            if (Predictions[i].className.indexOf(object.value)!=-1&&Predictions[i].probability>=probability.value) {
              objectCount++;
            }  
          }
